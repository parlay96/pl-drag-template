...
<script>
	...
	import {_pl_register_components_object} from '@/plugins/index'
  window._pl_register_components_object=_pl_register_components_object;
  console.log(_pl_register_components_object)
	export default {
		name: "component-libs",
		data() {
			return {
		...
		methods: {
			...
			getComponentProps(elName) {
				let elComponentData
				for (let key in _pl_register_components_object) {
					if (key.toLowerCase() === camelCase(elName).toLowerCase()) {
						elComponentData = _pl_register_components_object[key];
						break;
					}
				}
